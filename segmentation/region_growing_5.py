...
class Segment:
    def __init__(self, base_colour: np.ndarray):
...
def compare_colours(a: np.ndarray, b: np.ndarray) -> bool:
    return abs(int(a[0]) - int(b[0])) <= 40 \
        and abs(int(a[1]) - int(b[1])) <= 30 \
        and abs(int(a[2]) - int(b[2])) <= 30
...
while found_sth_to_analyse:
    ...
    while len(stack) != 0:
        ...
        if dr == 0:
            ...
            # if dr <= 0:  # left
            stack[l_][2] += 1
            if xx > 0 and status[yy, xx - 1] == 0 and compare_colours(seg.base_colour, arr[yy, xx - 1]):
                stack.append([yy, xx - 1, 0])
                continue
        if dr <= 1:  # top
            stack[l_][2] += 1
            if yy > 0 and status[yy - 1, xx] == 0 and compare_colours(seg.base_colour, arr[yy - 1, xx]):
                stack.append([yy - 1, xx, 0])
                continue
        if dr <= 2:  # right
            stack[l_][2] += 1
            if xx < (dim - 1) and status[yy, xx + 1] == 0 and compare_colours(seg.base_colour, arr[yy, xx + 1]):
                stack.append([yy, xx + 1, 0])
                continue
        if dr <= 3:  # bottom
            stack[l_][2] += 1
            if yy < (dim - 1) and status[yy + 1, xx] == 0 and compare_colours(seg.base_colour, arr[yy + 1, xx]):
                stack.append([yy + 1, xx, 0])
                continue
        stack.pop()
    segments.append(seg)
...
pickle.dump(status, open(os.path.join('segmentation', 'output', 'rg5_' + bitmap + '_status.pickle'), 'wb'))
pickle.dump(segments, open(os.path.join('segmentation', 'output', 'rg5_' + bitmap + '_segments.pickle'), 'wb'))
